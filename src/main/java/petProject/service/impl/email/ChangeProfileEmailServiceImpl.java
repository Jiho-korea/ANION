...
//	@Value("${mail.smtp.url.server}")
	...
	public boolean createChangeProfileEmail(String to_addr, String to_name, HttpServletRequest request, boolean isHtml,
		...
		mailHead = "<h1>[ANION] 이메일 변경 인증</h1><br><p>프로필에서 인증버튼을 클릭 후 인증번호 6자리를 입력하시면 이메일 변경이 완료됩니다.</p>";
		mailContent = emailcode + "<br><a href='" + url + request.getContextPath() + "/profile"
